...
import AnalyseDocuments from './pages/AnalyseDocuments';
...
function DocumentWatcher() {
  ...
  const handleNewDocument = useCallback((data) => {
    setNewDoc(data);
    toast(`📄 Nouveau document archivé : ${data.filename}`, 'ok');
  ...
  if (loading) {
    return (
      <div id="loader">
        ...
        <p style={{ color: 'rgba(255,255,255,.7)', fontSize: 14, letterSpacing: 1 }}>
  ...
  return (
    <BrowserRouter>
      ...
      <Routes>
        ...
        <Route path="/upload"
          element={<Protected droit="upload"><Layout><Upload /></Layout></Protected>} />

        <Route path="/documents"
          element={<Protected><Layout><Documents /></Layout></Protected>} />

        <Route path="/analyse-documents"
          element={<Protected><Layout><AnalyseDocuments /></Layout></Protected>} />

        <Route path="/search"
          element={<Protected><Layout><Search /></Layout></Protected>} />

        <Route path="/fleet"
          element={<Protected><Layout><Fleet /></Layout></Protected>} />

        <Route path="/checks"
          element={<Protected><Layout><Checks /></Layout></Protected>} />

        <Route path="/recent-uploads"
          element={<Protected><Layout><RecentUploads /></Layout></Protected>} />

        <Route path="/pipeline"
          element={<Protected><Layout><Pipeline /></Layout></Protected>} />

        <Route path="/pipeline-demo"
          element={<Protected><Layout><PipelineDemo /></Layout></Protected>} />

        <Route path="/monitoring"
          element={<Protected droit="monitoring"><Layout><Monitoring /></Layout></Protected>} />

        <Route path="/analytics"
          element={<Protected droit="analytics"><Layout><Analytics /></Layout></Protected>} />

        <Route path="/admin"
          element={<Protected droit="admin"><Layout><Admin /></Layout></Protected>} />
